...
- **No scroll hijacking for ambient/background effects.** Parallax is layer translation driven by scroll position only. No wheel interception, no `preventDefault` on scroll/wheel, no forced `scrollTop`, no scroll-linked timeline that blocks normal scrolling. **Approved exception (2026-07-27), narrowly scoped:** the user was shown a live comparison (fade vs. amplified vs. real pinning) and chose real "Apple-style" pinning for exactly **one** handoff — hero -> skills, where hero visually holds in place while skills slides up over it. This is implemented with `position: sticky` + `z-index` stacking only (`.pin-sec` in `app/globals.css`, wired up in `app/page.tsx`): no JS ever captures wheel/touch input or drives scroll position, so the scrollbar, mouse wheel, touch, and keyboard scrolling (Page Down, Space, arrows) all work exactly as normal throughout. This is deliberately **not** applied site-wide: the user explicitly rejected chaining pinned sections back to back, since several can end up visually stacked at once while scrolling through a run of them. Every other section boundary (skills->languages, languages->projects, projects->htb, htb->github, github->social, social->footer) instead uses a softer, non-pinning "amplified fade" — a stronger opacity/blur/scale falloff than the site's normal reveal, driven by a passive rAF-throttled scroll listener writing only `opacity`/`filter`/`transform` (`useAmpFade` in `app/lib/ampFade.ts`, `.amp-fade` in `app/globals.css`). Nothing sticks or stacks there. The underlying rule still stands for the ambient background parallax blobs — those remain translation-only, non-pinning, as described above.
